...
document.addEventListener('DOMContentLoaded', function() {
var noticiaId = obterParametroUrl('id');
if (noticiaId) {
carregarNoticiaIndependente(noticiaId);
...
function carregarNoticiaIndependente(id) {
var noticias = JSON.parse(localStorage.getItem('noticias')) || [];
var noticia = noticias.find(noticia => noticia.id == id);

if (!noticia) {
fetch('../assets/json/noticias.json')
    .then(response => response.json())
    .then(data => {
        noticia = data.noticias.find(noticia => noticia.id == id);
        if (noticia) {
...
document.querySelector('.titulo h2').textContent = noticia.titulo;
document.querySelector('.imagem_noticia img').src = noticia.imagem;
document.querySelector('.conteudo_noticia').innerHTML = noticia.conteudo.map(paragrafo => `<p>${paragrafo}</p>`).join('');
document.querySelector('.data_noticia p').textContent = dataFormatada;
document.querySelector('.autor h4').textContent = noticia.autor;
}
